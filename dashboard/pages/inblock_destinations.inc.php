...
function getInBlockDestinations(){
	...
	$page_html = '
		<div id="page-wrapper">
			<div class="row">
            	<div class="col-lg-12">
                	<h1 class="page-header">Inbound Number Manager</h1>
	';
	...
	$page_html .= '<form name="input" action="inblocked_destinations.php" method="post" >
	...
	if(count($page_vars['members'])>0){
		...
		$page_html .= '<caption>Inbound Numbers Not Found.</caption>';
	}
	
	$page_html .= '<tr class="title">
				<td width="17%" class="first td_order_DESC">Inbound Number</td>
				<td width="17%" class="td_order_ASC">Created</td>	
	...
	if(count($page_vars['members'])>0){
		foreach($page_vars['members'] as $one_member){
			$page_html .='
			<form action="inblocked_destinations.php" method="post" accept-charset="utf-8">
			<tr class="tr_highlight">				
				...
				<input type="submit" name="Edit"   value="Edit" class="logged-button" style="font-size:12px;" >
					<input type="submit" name="Delete"   value="Delete" class="logged-button" style="font-size:12px;"  onclick="javascript:return confirm(\'Are you sure you want to delete Inbound Number :'.$one_member['destination'].' ?\')" >
				</td>';
	...
	if($page_vars['page_count'] > 1){	
		...
		$se="'".$page_vars['Submit']."'";
		$page_html .= '<select name="paginate" onchange="javascript:block_pagination(\'inblocked_destinations\',this.value,'.$de.','.$se.')">'."\n";
...
function getUpdateInDestinationPage(){
	...
	$page_html .= '<div class="title-bg"><h1>Update Inbound Number</h1></div><div class="mid-section">
	               <div class="date-bar">
					<div class="left-info">Update Inbound Number '.$destiantion_number.'</div>
										</div>
	...
	<form name="input" action="inblocked_destinations.php" method="POST" >
	<table width="100%" border="0" cellspacing="0" cellpadding="7" class="configure-number">		
		...
		<tr>
			...
			<td width="75%">Enter Inbound Number.</td>
		</tr>
		';
