<?php
        ...
        if($_POST['action'] ==  'delete')
        {
            ...
            $chkQry     =   mysql_query("SELECT AcctId FROM cdr WHERE AcctId LIKE '$deleteID'");
            if(mysql_num_rows($chkQry) > 0)
            {
                $callInfo   =   mysql_fetch_object(mysql_query("SELECT * FROM cdr WHERE AcctId LIKE '$deleteID'"));
            
                $insQry     =   "INSERT INTO cdr_archieve SET
                calldate    =   '$callInfo->calldate',
                clid        =   '$callInfo->clid',
                src         =   '$callInfo->src',
                dst         =   '$callInfo->dst',
                dcontext    =   '$callInfo->dcontext',
                channel     =   '$callInfo->channel',
                dstchannel  =   '$callInfo->dstchannel',
                lastapp     =   '$callInfo->lastapp',
                lastdata    =   '$callInfo->lastdata',
                duration    =   '$callInfo->duration',
                billsec     =   '$callInfo->billsec',
                disposition =   '$callInfo->disposition',
                amaflags    =   '$callInfo->amaflags',
                accountcode =   '$callInfo->accountcode',
                userfield   =   '$callInfo->userfield',
                uniqueid    =   '$callInfo->uniqueid',
                test        =   '$callInfo->test',
                fwd_num     =   '$callInfo->fwd_num',
                per_min_cost=   '$callInfo->per_min_cost'";

                mysql_query($insQry);

                mysql_query("DELETE FROM cdr WHERE AcctId = '$deleteID'");
            }
        }
        
function getAsteriskErrorPage(){
	...
	$page_html.=' <div class="date-bar">
					<div class="left-info" style="float:right;"><b>
                                        <a href="admin_asterisk_archieve_errors.php"><input id="logged-btn" class="logged-button" type="submit" title="" style="background:#359DD1; padding:0px 6px 2px 6px; border-radius:4px; width:auto; margin-top:7px;" alt="" value="View Archieve" name="logged-btn"></a>
                                        &nbsp;
                                        </b></div>
                                        </div>
...
//	}
	...
	$page_html .= '<tr class="title">
				...
				<td width="20%" class="td_order_ASC">Caller ID</td>
				...
				<td width="10%" class="td_order_DESC">Member ID</td>
                                <td width="10%" class="td_order_DESC">Action</td>
			</tr>';
		...
		foreach($page_vars['my_cdrs'] as $one_cdr){
			...
			$page_html .='<tr >
				...
				<td>'.$one_cdr["disposition"].'</td>
                                <td>'.$memberID.'</td>
				<td><input id="logged-btn" class="logged-button" type="submit" value="Archieve" name="logged-btn" onclick="deleteRecord('.$one_cdr["callID"].')" style="background:#359DD1;border-radius:4px; width:auto;" ></td>    
			</tr>';
		}//end of foreach
	}//end of if count
	...
	if($page_vars['page_count'] > 1){	
		...
		$page_html .= '<select name="paginate" onchange="window.location = \'admin_asterisk_errors_mgr.php?page=\'+this.value">'."\n";
