<?php
	...
	if(is_object($assigned_did)==false){
  		echo "<br>Error: Failed to create DidwwApi object.";
  		exit;
  	}
	
	//Get the DIDWW Order ID of this DID
	$didww_order_id = $assigned_did->getDIDWWOrderIdFromDIDNumber($user_id,$did_number);
	if($didww_order_id == 0){
		echo "<br>Error: No record for DIDWWOrder found.";
		exit;
	}else{
		echo "<br>DIDWW order id is: ".$didww_order_id;
	}
	
	//Now call getservicedetails to get the expire date in GMT
	$did_order = $client->getservicedetails($user_id,$didww_order_id);
	if(is_object($did_order)==false){
		echo "<br>".$client->getError();
		exit;
	}
	if($did_order->result!=0){//Failed
		echo "<br>Error: Failed to getservicedetails. Result is: $did_order->result, Error Code is: $client->getErrorCode(), Error is: $client->getErrorString()";
		exit;
	}
