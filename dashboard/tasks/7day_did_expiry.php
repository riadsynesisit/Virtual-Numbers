...
//Get the DIDs that were assigned exactly on the 7 th day in past from the current date
$query = "select ad.id as id, 
			...
			from assigned_dids ad LEFT JOIN user_logins ul ON ad.user_id=ul.id 
			where 
				datediff(CURDATE(),ad.did_assigned_on) > 6 and 
				(SUBSTRING(ad.did_number,1,3)='070' OR SUBSTRING(did_number,1,3)='087') and 
				ad.did_activated_on is null and 
				ad.route_last_used is null
...
while($row=mysql_fetch_array($result)){
	...
	//Remove the record from the ast_rt_extensions table
	$delete_query = "delete from ast_rt_extensions where exten='".$row['did_number']."'";
	mysql_query($delete_query);
	if(mysql_affected_rows()>0){
		echo "\nDeleted did number ".$row['did_number']." from ast_rt_extensions.";
		$rows_ast_rt_extensions = $rows_ast_rt_extensions + 1;
	}else{
		echo "\nNo rows found for the above DID in ast_rt_extensions.";
	}
	
	//Remove the record from the assigned DIDs table
	$remove_query = "delete from assigned_dids where did_number='".$row['did_number']."'";
	mysql_query($remove_query);
	if(mysql_affected_rows()>0){
		echo "\nDeleted did number ".$row['did_number']." assigned on ".$row['did_assigned_on']." from assigned_dids.";
		...
		if($reallo_result=="SUCCESS"){
			//Add the number to the unassigned_dids table
			echo "\nSuccessfully DEAC and reALLO the DID: ".$row['did_number'];
			//Now update the date added field
			$insert_query = "insert into unassigned_dids (did_number, date_added) values ('".$row['did_number']."',now());";
			mysql_query($insert_query);
		}else{
			echo "\nDID ".$row['did_number']." could not be reALLOed after a DEAC";
		}
		...
		$deleted_dids_ins_query = "insert into deleted_dids(
						...
						'".$row["route_number"]."',
						NOW(),
						'Not called for 6 days'
					)";
		$deleted_dids_result = mysql_query($deleted_dids_ins_query);
		//Insert a record into access logs table
		$access_query = "insert into access_logs (user_id, type, type_id, id_address, item) values (".$row['user_id'].",'System',".$row['user_id'].",'178.79.166.188','Number ".$row['did_number']." removed because it was not called in 6 days.');";
		...
		$new_message->message_title = "Unused DID Number Removed";
		$new_message->message_body = "Your registered DID ".$row['did_number']." was removed from your account on ".date("D, d M Y",time())." because it had not been used within six days of registration.";
...
//Save the number of DIDs removed in the daily_system_stats.7day_dids_removed table
$daily_stats = new daily_stats();
$daily_stats->updatePendingDIDRemovals($rows_assigned_dids);
