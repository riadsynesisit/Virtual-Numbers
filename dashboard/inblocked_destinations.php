...
function show_delete_page($this_page){
	...
	global $action_message;
	$assigned_did = new In_Block_Destination();
	
	$status = $assigned_did->deletedestinationData($_POST['destiantion_number']);
	if($status==true)
	{
		$action_message .= "<h2>Success! </h2>\n";
			$action_message .= "<p>We successfully Deleted  your Inbound Number '".$_POST['destiantion_number']."'.</p>\n";
			process_search($this_page);
	}
	else
	{
	        $action_message .= "<h2>Problems...</h2>\n";
			$action_message .= "<p>We encountered some errors in your information when saving your changes to your Inbound Number '".$_POST['destiantion_number']."'.</p>\n";
...
function process_edit_page($this_page){
	...
	global $action_message;
	$this_member = new In_Block_Destination();
	...
	$action_message .= "<p>ERROR: Same Inbound Number Already Exists.</p>\n";
	show_edit_page($this_page);
	}
	else
	{
	...
	{
		$action_message .= "<h2>Success! </h2>\n";
			$action_message .= "<p>We Successfully Updated  your Inbound Number '".$_POST['destiantion_number']."'.</p>\n";
			process_search($this_page);
	}
	else
	{
...
function show_edit_page($this_page){
	...
	$page_vars['menu_item'] = 'inblock_destination';
	...
	$this_page->content_area .= getUpdateInDestinationPage();
	
	common_header($this_page, "Edit Inbound Number - ".$_POST['destiantion_number']);
...
function process_search($this_page)
{
	...
	if($_REQUEST['Add']=='Add')
		...
		if($ck!=0)
		{
		 $action_message .= "<p>ERROR: Adding Inbound Number Already Exists.</p>\n";
		...
		if($st==true)
		{
		 $action_message .= "<p>SUCCESS: Inbound Number Added Success.</p>\n";
		}
		else
		{
	...
	$this_page->content_area .= getInBlockDestinations();//In pages/inblock_destinations.inc.php
	
	common_header($this_page,"'Blocked Inbound List'");
