<?php
    ...
    if($_SESSION['admin_flag']!=1 && $_SESSION['admin_flag']!=2){
    	echo "Error: Unauthorised to access blacklisting page.";
    	exit;
    }
    ...
    switch($input_type){
    	case "dest_number":
    		...
    		$status = $blocked_destination->add_blocked_destination($input_value);
    	...
    	case "ip_address":
    		...
    		$status = $blacklist_ip->black_list_ip($input_value,$user_id);
    		if($status!==true && substr($status,0,6)=="Error:"){
    			Echo $status;
    			exit;
    		}
